...
preds <- read.csv("./results/mouse/all_cell_lines/lgbm-LL_P5_P3_CF_AAF_3mer_freq_5/predictions.csv", 
  header = TRUE, sep = ","
)
...
hist_data <- preds[,c("brain_true", "mean_across_cell_lines_true")]
hist_plot2 <- ggplot(hist_data, aes(x = mean_across_cell_lines_true, y = brain_true)) +
    geom_bin2d(bins = 100) +
    labs(x = "Mean TE", y = "brain TE", title = NULL) +
    scale_fill_continuous(type = "viridis", limits = c(0,50)) +
    # stat_cor(method = "pearson", label.x = -2, label.y = 2, label.sep = "\n", size = 3) +
    stat_cor(method = "spearman", cor.coef.name = "rho", label.x = -2, label.y = 2, label.sep = "\n", size = 3) +
    theme_minimal() +
...
ggsave("./graphing/mouse/C/graph_pred_vs_true_mean_te_and_brain.pdf", combo_hist, width = 10, height = 4.75, units = "in", dpi = 300, bg = 'white')