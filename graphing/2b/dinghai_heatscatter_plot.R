...
predictions <- read.csv("./results/human/all_cell_lines/lgbm-LL_P5_P3_CF_AAF_3mer_freq_5/predictions.csv", 
  header = TRUE, sep = ","
)
...
pdf("./graphing/C/predicted_vs_observed_mean_TE_human.pdf", 5, 5) 
...
pdf("./graphing/C/HEK293T_vs_observed_mean_TE_human.pdf", 5, 5) 
# 'predictions' below is a dataframe with at least two columns named PredictedMeanTE and ObservedMeanTE
predictions %>% 
  mutate(
    r = cor(bio_source_HEK293T_true, ObservedMeanTE, use = "complete.obs"),
    rho = cor(bio_source_HEK293T_true, ObservedMeanTE, method = "spearman", use = "complete.obs"),
    ) %>%
  {
    heatscatter(
      .$bio_source_HEK293T_true, .$ObservedMeanTE,
      xlim = c(-4, 4),
      xlab = "HEK293T TE",
      ylab = "Mean TE", bty='n', cex=0.3, las=1,
      main = str_glue("r = {.$r[1] %>% round(3)}, rho = {.$rho[1] %>% round(3)}")
